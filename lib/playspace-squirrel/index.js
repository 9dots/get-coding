...
function playspaceSquirrelCtrl($element, $timeout, $mdBottomSheet) {
  ...
  function right(){
    if(vm.buttons && window.buttonPressed != true){
      ...
      var message = "CAN'T CALL right() HERE";
      var subMessage = "You need to write a function called \`rightButton()\` and set it to Button " + getButtonNumber("right") + " with \`setButton(" + getButtonNumber("right") + ", rightButton)\`.";
  ...
  function left(){
    if(vm.buttons && window.buttonPressed != true){
      var button = vm.buttons.indexOf()
      var message = "CAN'T CALL left() HERE";
      var subMessage = "You need to write a function called \`leftButton()\` and set it to Button " + getButtonNumber("left") + " with \`setButton(" + getButtonNumber("left") + ", leftButton)\`.";
      buttonError(message, subMessage)
    } else {
  ...
  function up(){
    if(vm.buttons && window.buttonPressed != true){
      ...
      var subMessage = "You need to write a function called \`upButton()\` and set it to Button " + getButtonNumber("up") + " with \`setButton(" + getButtonNumber("up") + ", upButton)\`.";
      buttonError(message, subMessage)
    } else {
  ...
  function down(){
    if(vm.buttons && window.buttonPressed != true){
      ...
      var subMessage = "You need to write a function called \`downButton()\` and set it to Button " + getButtonNumber("down") + " with \`setButton(" + getButtonNumber("down") + ", downButton)\`.";
      buttonError(message, subMessage)
    } else {
  ...
  function move(num){
    if(vm.buttons && window.buttonPressed != true){
      var button = vm.buttons.indexOf()
      var message = "CAN'T CALL move() HERE";
      var subMessage = "You need to write a function called \`moveButton()\` and set it to Button " + getButtonNumber("move") + " with \`setButton(" + getButtonNumber("move") + ", moveButton)\`.";
      buttonError(message, subMessage)
    } else {
      if(vm.failed === false) {
        $(".squirrel").show();
  ...
  function loop (n, fn) {
    let char = chars[vm.loops++ % 3]
    if (typeof (n) === 'function') {
      n = fnString(n)
      return `for (var ${char} = 0; ${char} < 100; ${char}++) { ${n} }`
    } else {
      fn = fnString(fn)
      return `for (var ${char} = 0; ${char} < ${n}; ${char}++) { ${fn} }`
    }
  }
  ...
  function getAcorn(){
    if(vm.buttons && window.buttonPressed != true){
      var button = vm.buttons.indexOf()
      var message = "CAN'T CALL getAcorn() HERE";
      var subMessage = "You need to write a function called \`getAcornButton()\` and set it to Button " + getButtonNumber("getAcorn") + " with \`setButton(" + getButtonNumber("getAcorn") + ", getAcornButton)\`.";
      buttonError(message, subMessage)
    } else {
      if(vm.failed === false) {
        vm.field.hideAcorns(vm.squirrel.acorns);
  ...
  function getButtonNumber(name){
    var ind;
    _.each(vm.buttons, function(button, index){
      if(button.name === name){
        ind= index + 1;
      }
    })
  ...
  vm.squirrel = {
    ...
    get: function(){
      ...
      if (vm.field.isAcorn(vm.squirrel.squareOn)){
        //add check to see if buttons were programmed and/or pushed
