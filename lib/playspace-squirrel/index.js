...
function playspaceSquirrelCtrl($element, $timeout, $mdBottomSheet) {
  ...
  function setFacing(){
    if(vm.squirrel.facing === "onTree" && isNotEmpty(vm.squirrel.squareOn)){
  ...
  function recordPostion(){
    ...
    if(vm.squirrel.facing === 'onTree' && isGround(check)){
      vm.squirrel.canMove = false;
    } else {
      vm.squirrel.canMove = isNotEmpty(check);
    }
