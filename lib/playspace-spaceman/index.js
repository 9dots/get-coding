...
function playspaceSpacemanCtrl (appRunner, $element, $timeout) {
  var vm = this
  vm.moving = false
  var error = {}
  vm.running = false
  ...
  function numParts () {
    if (vm.field.isPart(vm.astronaut.position)) {
      return vm.field.byId[vm.astronaut.position].count || 0
    }
    return 0
  }
  ...
  vm.field = {
    init: function(options) {
      ...
      this.setPlanets(options.planets)

      var planetParts = _.reduce(this.byId, function (sum, square, key) {
        if (square.type === 'part' && square.option === 'planet') {
          sum += square.count
        }
        return sum
      }, 0)
      ...
      this.pickedUp = 0
      this.toPickUp = options.parts.length + planetParts
    },
    ...
    setBlocks: function(blocked) {
      ...
      _.each(blocked, function(block) {
        if(block[1]  === '*') {
          ...
          for (var i = Number(block[0]); i <= Number(block[3]); i++) {
            for (var j = Number(block[1]); j <= Number(block[4]); j++) {
              var id = '' + i + j
              self.setBlocked(id)
            }
          }
    ...
    setParts: function(parts) {
      ...
      _.each(parts, function(part) {
        if (self.byId[part].type === 'blocked') {
          lose("You can't put a part on square " + part + " because is it blocked.")
        } else if (self.byId[part].type === 'part') {
          self.byId[part].count++
        } else {
          self.setPart(part)
          self.byId[part].type = 'part'
          self.byId[part].count = 1
        }
      })
    },
    setPlanets: function (planets) {
      ...
      _.each(planets, function (planet) {
        if (self.byId[planet].type === 'blocked') {
          lose("You can't put a part on square " + planet + " because is it blocked.")
        } else {
          self.setPlanet(planet)
          self.byId[planet].count = Math.round(Math.random())
        }
      })
    },
    numRows: function() {
    ...
    setPlanet: function (pos) {
      this.byId[pos].type = 'part'
      this.byId[pos].option = 'planet'
    },
    setEmpty: function (pos) {
      this.byId[pos].type = 'empty'
      this.byId[pos].option = ''
    },
    takePart: function (pos) {
      if (this.byId[pos].count === 0) {
        return lose('There is no satellite part on this square.')
      }
      this.byId[pos].count--
      this.pickedUp++
      if (this.byId[pos].count === 0) {
        if (this.byId[pos].option === 'solar-panel') {
          this.setEmpty(pos)
        }
      }
    },
