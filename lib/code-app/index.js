...
angular.module(name, ['ui.router', require('lib/div-resize'), require('lib/marked')])
...
.controller('AppCtrl', function(appRunner, apps, $location, $rootScope, $mdDialog, $timeout, $mdSidenav) {
  var vm = this;
  ...
  function save() {
    vm.workspaces[vm.level] = vm.Workspace.toJSON();
    //save for 60 minutes
    lscache.set(appKey, {
  ...
  function checkError() {
    $timeout(function() {
      var e = vm.Playspace.api.err && vm.Playspace.api.err()
      if (! _.isEmpty(e)) {
        appRunner.error(e, true);
      }
    }, 50);
  }
  ...
  function showFinish(level) {
    ...
    $mdDialog.show({
      ...
      controller: function($scope) {
        ...
        $scope.content = "##You passed level " + level+"!\n You wrote " + vm.locUsed + " lines of code!";
      }
    })
  }

  function showHelp() {
    var help = getLevel().help;
    if (!help) return;

    // first value is special
    var content = _.values(help)[0];
  ...
  function setHelp() {
    ...
    vm.sidebar.tabs = [];
    if (!level.help) {
      vm.sidebar.tabs.push({title: 'Directions', content: app.help});
    } else {
      _.each(level.help, function(value, key) {
        vm.sidebar.tabs.push({title: key, content: value});
      })
    }
  }
