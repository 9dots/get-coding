...
var analyse = require('/lib/analyse-code');
...
.controller('AppCtrl', function(appRunner, apps, $location, $rootScope, $mdDialog, $timeout) {
  ...
  function activateWorkspace() {
    ...
    analyseCode(true);
    vm.Workspace.on('change', function() {
      analyseCode(true);
      $rootScope.$apply();
    });
  }
  ...
  function setLevel(levelNum, includeWorkspace) {
    ...
    if (includeWorkspace && level && level.workspace && !_.isUndefined(level.workspace.codeSpace)) {
  ...
  function analyseCode(ignoreError) {
    try {
      vm.codeAnalysis = analyse(vm.Workspace.toCode());
    } catch (e) {
