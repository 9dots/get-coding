...
.controller('AppCtrl', function(appRunner, apps, $firebaseObject, $location, $rootScope, $mdDialog, $timeout, $mdSidenav, $mdBottomSheet) {
  ...
  function setLevel(levelNum, includeWorkspace) {
    ...
    vm.availableApi = level.workspace ? level.workspace.visibleApi : undefined
  ...
  function analyseCode(ignoreError) {
    try {
      ...
      vm.apiCounter = analysis.expressionCount.bind(analysis)
      vm.locUsed = analysis.lloc()
      appRunner.set('apiCounter', vm.apiCounter)
    } catch (e) {
  ...
  appRunner.on('run', function() {
    ...
    _.each(vm.availableApi, function (fn) {
      if (fn.allowed && vm.apiCounter(fn.name) > fn.allowed) {
        var strs = ['times', 'are']
        if (fn.allowed > 1) {
          strs[0] = 'time'
        }
        if (vm.apiCounter(fn.name) > 1) {
          strs[1] = 'is'
        }
        return appRunner.error({
          toString: function() {
          ...
          subMessage: 'You used ' + fn.name + ' ' + vm.apiCounter(fn.name) + ' ' + strs[0] + '. Only ' + fn.allowed + ' ' + strs[1] + '  allowed.'
        });
      }
    })
  });
