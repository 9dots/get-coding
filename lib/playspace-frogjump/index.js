...
function playspaceFrogjumpCtrl($element, $timeout) {
  ...
  function activate(level, speed) {
    ...
    error = {};
    vm.field.init(_.cloneDeep(level));
  ...
  function frogJump() {
    ...
    if (pos[0] >= vm.field.grid.length || pos[0] < 0) {
      ...
      vm.failed = {
        toString: function(){
          return 'No lilypad that way!'
        },
        subMessage: "That frog can't move further without falling off the lilypads!"
      }
      ...
      vm.failed = {
        ...
        subMessage: "That frog can't jump the frog without falling off the lilypads!"
      }
  ...
  vm.field = {
    ...
    directFrogs: function(){
      ...
      for(var i=0; i<this.frogs.length; i++){
        ...
        this.frogs[i].checkWin = checkWin;
