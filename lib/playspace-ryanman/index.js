var name = module.exports = 'playspsace-ryanman';
...
angular.module(name, [])
.directive('playspaceRyanman', function() {
  return {
    restirct: 'A',
    controller: playspaceRyanmanCtrl,
    controllerAs: 'Ryanman',
...
//everything above stays...make changes below
function playspaceRyanmanCtrl($element, $timeout) {
  var vm = this;
  vm.moving = false;

  // playspace interface..these four necessaary for all playspaces
  ...
  //user has access to everything in api
  function api() {
    return {
      getPart: function() {
        vm.astronaut.get();
