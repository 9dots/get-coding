...
.directive('workspaceAce', function() {
  return {
    ...
    controller: function($scope, appRunner) {
      ...
      function updateApi () {
        var level = _.clone($scope.$parent.App.level)
        var api = _.clone($scope.$parent.App.levels[level].workspace.visisbleApi)
        var apiCalls = appRunner.get('apiCalls')
        console.log(api, apiCalls)
        $scope.api = _.map(api, function (obj) {
          if (apiCalls) {
            obj.allowed = obj.allowed ? obj.allowed - (apiCalls[obj.name] || 0) : undefined
          }
          return obj
        })
      }
