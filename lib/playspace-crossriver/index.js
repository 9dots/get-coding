...
function playspaceCrossriverCtrl($element, $animate) {
  ...
  function getCaptain(){
    var captain = vm[vm.boat.position].get(vm.boat.captain)
  ...
  function CROSSRIVER(){
    if(vm.boat.captain && vm[vm.boat.position].get(vm.boat.captain))
      getCaptain();
    ...
    for (var i = 0; i < this.characters.length; i++) {
      ...
      for (var j = 0; j < keys.length; j++) {
        if (keys[j] === 'prey')
          ...
          this.checkTypes(this.characters[i],this.characters[i].rules[keys]);
      }
    }
  }
