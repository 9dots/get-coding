...
function playspaceCrossriverCtrl($element, $animate) {
  ...
  function resetItems(){
    $(".front-character").removeClass("deadchicken");
    $(".front-character").removeClass("deadgrain");
    $(".back-character").removeClass("deadchicken");
    $(".back-character").removeClass("deadgrain");
    $(".front-character").show();
    $(".back-character").show();
  }
  ...
  function checkPrey(predator, key){
    ...
    if (dead) {
      ...
      console.log(prey);
      $("." + dead.type).addClass("dead" + prey);
      console.log(predator);
      $("." + predator).hide();
      vm.failed = {
        toString: function(){
          return 'The ' + predator + ' ate the ' + prey + '!';
        }, 
        subMessage: "You can't leave the " + predator + " alone with the " + prey + "!"
      }
    }
  }
      ...
      for (var j = 0; j < keys.length; j++) {
        if (keys[j] === 'prey' && !this.hasCharacter(vm.boat.captain))
          ...
          if(results[0] < results[1]) {
            vm.failed = {
              toString: function(){
              return 'Too many ' + listCharacters(characters[i].rules[keys]) + "s on one side.";
              }, 
              subMessage: "You can't leave the " + predator + " alone with the " + prey + "!"
            }
          }
