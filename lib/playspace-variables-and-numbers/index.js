  ...
  function check(variable){
    ...
    if(vm.requiredCalls){
          ...
          if(req != 0){
            if(made === 0 && vm.checking){
              ...
              vm.failed = {
                ...
                subMessage: "You have not call all the variables you need to call in this level."
              }
              vm.checking = false;
            } 
          }
      ...
      if (!vm.answer){
        vm.counting = true;
        for (var i=0; i<vm.variableNames.length; i++){
          if(vm.variables[vm.variableNames[i]] === vm.values[i]) {
            count +=1;
          }
        }
        vm.counting = false;
  ...
  /*
        _.each(vm.variableNames, function(name, index){
          if (vm.variables[vm.variableNames[index]] === null && vm.checking) {
            var message = "You didn't declare the variable " + vm.variableNames[index];
            vm.failed = {
              toString: function(){
